...
cas97_glm<-cas97 %>% select(grep("^G2|^G3|^G4|^G8|^G9|^G10|^G15|^G14|^G16|^G17|^G13|^A6|^A8| #family/HS and living sit
  ...
  mutate(DRINKCAT=as.factor(DRINKCAT)) %>%
  drop_na()
naive_lasso<-cv.glmnet(x=cas97_glm %>% select(-DRINKCAT) %>% mutate_each(as.numeric) %>% as.matrix(), 
       y=cas97_glm %>% select(DRINKCAT) %>% mutate_each(as.factor) %>% as.matrix(),family="multinomial")
coef(naive_lasso)
```
...
modb <- "
  ...
  # Latent variable definition
  CollegePolicy =~ B1 + B2 + 
                   #B6_A + B6_B + B6_C + B6_D + B6_E +
                   B9_A 
                   #+ B9_B + B9_C + B9_D + B9_E + B9_F + B9_G
  CityPolicy =~ #B10_1 + B10_2 + B10_3 + B10_4 + B10_5 + B10_6 + B10_7 + B10_8 + # excluded because no variance
                B11 + B12_A 
                #+ B12_B + 
                B13_A 
                #+ B13_B + B13_C + B13_D + B13_E highly correlated
  CollegeEdu =~ B7_A 
                #+ B7_B + B7_C + B7_D + B7_E + B7_F +
                B8_B #+ B8_C + B8_D 
                
                #+ B8_E + B8_A  # excluded because no variance
  # Variance
...
DRINKCAT <- ordered(cas97$DRINKCAT)
drinkcat <- cas97 %>%
  select(grep("^F5_A|^F5_B|^F5_C|^F5_D|^F5_E|^F5_F|^F5_G|^F5_H|^F6_A|^F6_B|^F6_C|^F6_D|^F6_E|^F6_E|^F6_F|^F6_G|^F6_H|^F6_I|^F1|^F4|^F2|^F3|^G2|^G3|^G4|^G8|^G9|^G10|^G15|^G14|^G16|^G17|^G13|^A6|^A8|^D2|^D8|^D9|^D3|^D1|^A9|^A10|sex_assault_ind", colnames(.))) %>%
  mutate(G15 = scale(G15),
         G16 = scale(G16),
         G17 = scale(G17),
         G10 = scale(G10),
