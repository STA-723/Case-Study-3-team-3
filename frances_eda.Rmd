...
elastic net model using the $\texttt{glmnet}$ package. 


```{r cars}
cas93<-readRDS("cas93.rds")
cas93 %>% mutate_at(vars(starts_with("B1_")),funs(factor)) %>% dplyr::select(starts_with("B1_")) %>% count(B1_E)
```
...
#   dplyr::select(-COLL_ID,-STUDY_ID,-A1)
# 
...
```
```{r}
C_potential_qs<-MICEd_impute_C %>% dplyr::select(grep("C10|C14|C16|C17|C21",colnames(MICEd_impute_C)))
G_potential_qs<-MICEd_impute_C %>% dplyr::select(grep("G2|G3|G8|G11|G12",colnames(MICEd_impute_C)))

C_potential_qs<-cas97 %>% dplyr::select(grep("C10|C14|C16|C17|C21",colnames(.))) %>% drop_na()
colnames(C_potential_qs)<- c("last_drink","Dorm_party","Frat_Sor","On_campus_dance","Home_athletic","Away_athletic","On_campus_pub","Off_campus_party","Off_campus_bar",
                             "Hangover","miss-class","behind_schoolwork","regret","forgot", "argue", "uplanned_sex", "unprotected_sex","damage_property","police_trouble","hurt","alcohol_overdose","victim_on_campus","victim_off_campus",
                             "warning","fine","alc_education","community_service","alcohol_treatment","other",
                             "get_away","drunk","friends","dating","reward","peer_pressure")

plot_cats<-function(filt,name_col,title,col_names) {
  C<-C_potential_qs %>% 
  dplyr::select(grep(filt,colnames(.))) %>%
  pivot_longer(cols=everything(),names_to=name_col)

ggplot(C,aes_string(x=name_col))+geom_bar()+
  facet_wrap(value~.,scales="free") +
  ggtitle(title) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_x_discrete(labels=col_names)
}

reasons_plot<-plot_cats("C21","reasons","Reasons for Drinking Faceted by Importance", c("C21_A"="get_away","C21_B"="drunk","C21_C"="friends","C21_D"="dating","C21_E"="reward","C21_F"="peer_pressure"))

ggsave("reasons_plot.png",reasons_plot)
```
One means very important, 4 means not important. 
```{r}
c("C14_A"="Dorm_party","C14_B"="Frat_Sor","C14_C"="On_campus_dance","C14_D"="Home_athletic","C14_E"="Away_athletic","C14_F"="On_campus_pub","C14_G"="Off_campus_party","C14_H"="Off_campus_bar")
num_drink_plot<-plot_cats("C14","events","Event Types attended, Faceted by #Drinks",c("C14_A"="Dorm_party","C14_B"="Frat_Sor","C14_C"="On_campus_dance","C14_D"="Home_athletic","C14_E"="Away_athletic","C14_F"="On_campus_pub","C14_G"="Off_campus_party","C14_H"="Off_campus_bar"))
ggsave("num_drink_plot.png",num_drink_plot)

```

```{r}
C14<-C_potential_qs %>% 
  dplyr::select(grep("C14",colnames(.))) 
...
```{r}
C21<-C_potential_qs %>% 
  dplyr::select(grep("C21",colnames(.))) 
colnames(C21)<-"change"
...
```{r PCA-attempt}
A<-MICEd_impute_C %>%
  dplyr::select(grep("^A[0-9]",colnames(.)))
...
MICEd_impute<-cas93 %>% 
  ...
  mutate(ones=1) %>%
  pivot_wider(names_from=A6,values_from=ones, values_fill=list(ones=0),names_prefix = "A6_")  %>%  #indicators for roommates  
  dplyr::select(grep("^G2|^G3|^G4|^G8|^G9|^G10|^G15|^G14|^G11|^G12|^G13|^A5|^A6",colnames(.)))
